...
This utility will automatically install multiple instances of the extension in parallel, reducing deployment time from `~5m * N (extensions)` to just `~5m`. It also takes care of backloading existing Firestore data, and creating typed BigQuery Views by auto-generating schema definitions for your existing Collecitons.
